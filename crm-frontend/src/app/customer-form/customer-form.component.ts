...
export class CustomerFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.id != -1)
      this.service.retrieveCustomer("rajatgaurav", this.id).subscribe(
  ...
  onSave() {
      if(this.id == -1) {
        ...
        this.service.addCustomer("rajatgaurav", this.customer).subscribe(
          response => {
        ...
        this.service.updateCustomer("rajatgaurav", this.customer).subscribe(
          response => {
            console.log(response);
            this.router.navigate(['customers']);
          }
